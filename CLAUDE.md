...
git add -u testdata/vhs/
```

`testdata/vhs/*.png` contains committed visual baselines (currently only
`board-initial.png`). Use `git add -u` rather than a glob so that only
already-tracked baselines are staged — other screenshots generated by the
tapes are CI-only artifacts and should not be committed.
